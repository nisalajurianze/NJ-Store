...
const isValidRect = (rect) => Boolean(rect
...
const buildMorphState = (originRect, finalRect) => {
...
export const Modal = ({ children, isOpen, onClose, title, size = 'md', contentClassName, bodyClassName, overlayClassName, panelProps, originRect, anchorToOrigin = false, anchoredOffset = 14, morphOnClose = true, closeOnBackdropPointerDown = true, lockBodyScroll = true, ariaModal = true, overlayVariant = 'default', showHeader = true, performanceMode = 'default' }) => {
    const reduceMotion = useReducedMotion();
    const isFastMode = performanceMode === 'fast';
    const [isMounted, setIsMounted] = useState(false);
    const [morphState, setMorphState] = useState(null);
    const [anchoredPlacement, setAnchoredPlacement] = useState(null);
    const [isMorphReady, setIsMorphReady] = useState(false);
    const contentRef = useRef(null);
    const controls = useAnimationControls();
    const isBackdropPointerDown = (target) => {
    ...
    useLayoutEffect(() => {
        ...
        const anchoredLeft = clamp(originRect.left + originRect.width / 2 - anchoredWidth / 2, viewportPadding, window.innerWidth - anchoredWidth - viewportPadding);
        const anchoredTop = clamp(originRect.top + originRect.height + anchoredOffset, viewportPadding, window.innerHeight - anchoredHeight - viewportPadding);
        ...
        setAnchoredPlacement(anchorToOrigin
            ? {
                left: anchoredLeft,
                top: anchoredTop,
                width: anchoredWidth
    ...
    return createPortal(_jsx(AnimatePresence, { initial: false, mode: "wait", children: isOpen ? (_jsx(motion.div, { className: cn(overlayVariant === 'transparent'
                ...
                : overlayVariant === 'plain'
                    ...
                    : 'fixed inset-0 z-50 overflow-x-hidden overflow-y-auto bg-black/72 p-2 backdrop-blur-[6px] sm:p-6 lg:p-8', overlayClassName), role: "dialog", "aria-modal": ariaModal, "aria-label": title, initial: reduceMotion
                ? false
                : overlayVariant === 'default'
                    ? { opacity: 0, backdropFilter: 'blur(0px)' }
                    : overlayVariant === 'transparent'
                        ? { opacity: 0 }
                        : { opacity: 0 }, animate: reduceMotion
                ? undefined
                : overlayVariant === 'default'
                    ? { opacity: 1, backdropFilter: 'blur(6px)' }
                    : overlayVariant === 'transparent'
                        ? { opacity: 1 }
                        : { opacity: 1 }, exit: reduceMotion
                ? undefined
                : overlayVariant === 'default'
                    ? {
                        opacity: 0,
                ...
                if (closeOnBackdropPointerDown && isBackdropPointerDown(event.target)) {
                    onClose();
            ...
            }, children: _jsx("div", { className: "flex min-h-full min-w-0 items-start justify-center sm:items-center", children: _jsx(motion.div, { ref: contentRef, initial: shouldUseMorph ? false : reduceMotion ? false : { opacity: 0, y: 20, scale: 0.99 }, animate: shouldUseMorph ? controls : reduceMotion ? undefined : { opacity: 1, y: 0, scale: 1 }, exit: shouldUseMorphOnClose
                        ? {
                            opacity: 1,
                            x: morphState?.x ?? 0,
                            y: morphState?.y ?? 0,
                        ...
                        : reduceMotion
                            ? undefined
                            : { opacity: 0, y: 16, scale: 0.99, pointerEvents: 'none' }, transition: shouldUseMorph || reduceMotion ? undefined : motionTiming.modal, ...panelRestProps, style: {
                        borderRadius: DEFAULT_RADIUS,
                        ...(panelStyle ?? {}),
                        ...(shouldHideUntilMorphReady ? { visibility: 'hidden' } : {}),
                        ...(anchoredStyle ?? {})
                    }, className: cn('flex max-h-[min(880px,calc(100dvh-1rem))] w-full min-w-0 flex-col overflow-hidden rounded-[30px] border border-[color:var(--app-modal-border,rgba(255,255,255,0.10))] bg-[var(--app-modal-surface,#03050b)] text-[var(--app-modal-text,#f8fafc)] shadow-[var(--app-modal-shadow,0_28px_70px_rgba(0,0,0,0.42))] sm:max-h-[min(880px,calc(100dvh-2rem))]', sizeClasses[size], contentClassName, panelClassName), children: _jsxs(motion.div, { className: "contents", ...contentMotion, children: [showHeader ? (_jsxs("div", { className: "flex min-w-0 items-center justify-between gap-3 border-b border-[color:var(--app-modal-border,rgba(255,255,255,0.10))] bg-[var(--app-modal-header-surface,rgba(255,255,255,0.02))] px-4 py-4 sm:px-7 sm:py-5", children: [_jsx("h2", { className: "min-w-0 break-words font-display text-[1.35rem] leading-tight text-[var(--app-modal-text,#f8fafc)] sm:text-[1.8rem] sm:leading-none", children: title }), _jsx(Button, { variant: "ghost", size: "sm", className: "shrink-0 text-[var(--app-modal-muted,rgba(226,232,240,0.74))] hover:bg-[var(--app-modal-control-hover,rgba(255,255,255,0.07))] hover:text-[var(--app-modal-text,#f8fafc)]", onClick: onClose, "aria-label": "Close modal", children: _jsx(X, { className: "h-4 w-4" }) })] })) : null, _jsx("div", { className: cn('min-w-0 overflow-y-auto overscroll-contain px-4 pb-6 pt-5 sm:px-7 sm:pb-7 sm:pt-6', !showHeader && 'px-0 pb-0 pt-0 sm:px-0 sm:pb-0 sm:pt-0', bodyClassName), children: children })] }) }) }) })) : null }), document.body);
};
