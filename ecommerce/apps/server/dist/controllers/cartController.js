...
const resolveGuestSession = (req, res) => {
    const current = req.cookies.sessionId;
    if (current) {
        return current;
    }
...
export const addToCart = catchAsync(async (req, res) => {
    const data = await cartService.addItem(req.user?.id, req.user ? undefined : resolveGuestSession(req, res), req.body.productId, req.body.quantity, req.body.variantIndex);
    sendResponse(res, 201, data, 'Item added to cart');
});
export const updateCartItem = catchAsync(async (req, res) => {
    const data = await cartService.updateItem(req.user?.id, req.user ? undefined : resolveGuestSession(req, res), String(req.params.itemId), req.body.quantity);
    sendResponse(res, 200, data, 'Cart updated');
});
export const removeCartItem = catchAsync(async (req, res) => {
    const data = await cartService.removeItem(req.user?.id, req.user ? undefined : resolveGuestSession(req, res), String(req.params.itemId));
...
export const syncCart = catchAsync(async (req, res) => {
    const data = await cartService.syncCart(req.user.id, req.body.items, req.cookies.sessionId);
