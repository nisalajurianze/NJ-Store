...
const blockedProductionPrefixes = ['njstore-dev-', 'replace-with-'];
const usesBlockedProductionValue = (value) => blockedProductionPrefixes.some((prefix) => value.startsWith(prefix));
if (isProduction) {
    ...
    if (blockedProductionKeys.length > 0) {
        throw new Error(`Production environment variables must not use development or placeholder values: ${blockedProductionKeys.join(', ')}.`);
    }
}
