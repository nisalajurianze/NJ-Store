...
const createUploader = (maxSizeInMb, allowedMimeTypes, maxFileCount) => multer({
    storage: diskStorage,
    limits: {
...
export const collectUploadedFiles = (req) => {
    if (req.file) {
        return [req.file];
    }
    if (!req.files) {
...
export const requireFile = (fieldName) => (req, _res, next) => {
