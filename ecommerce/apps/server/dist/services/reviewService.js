...
export const reviewService = {
    listProductReviews: async (productId, viewerId) => {
        const reviews = await Review.find({ product: productId, isApproved: true })
            .populate('user', 'name')
            .sort({ createdAt: -1 });
        const viewerHelpfulVoteReviewIds = viewerId
            ? new Set((await ReviewHelpfulVote.find({
                user: new Types.ObjectId(viewerId),
                review: { $in: reviews.map((review) => review._id) }
            })
                .select('review')
                .lean()).map((vote) => vote.review.toString()))
            : undefined;
        return reviews.map((review) => serializeReview(review.toObject(), viewerHelpfulVoteReviewIds ? { viewerHasHelpfulVote: viewerHelpfulVoteReviewIds.has(review._id.toString()) } : undefined));
    },
    createReview: async (userId, payload) => {
        ...
        return serializeReview(review.toObject(), { viewerHasHelpfulVote: false });
    },
    toggleHelpfulVote: async (reviewId, userId) => {
        ...
        return serializeReview(review.toObject(), { viewerHasHelpfulVote });
    },
    ...
    replyToReview: async (reviewId, adminReply) => {
        const review = await Review.findByIdAndUpdate(reviewId, { adminReply: adminReply.trim(), adminRepliedAt: new Date() }, { new: true }).populate('user', 'name');
        if (!review) {
            throw new AppError('Review not found', 404);
        }
        return serializeReview(review.toObject());
    ...
    listPendingReviews: async () => {
        const reviews = await Review.find({ isApproved: false })
            ...
            .sort({ createdAt: -1 });
        return reviews.map((review) => serializeReview(review.toObject()));
    }
};
