...
const getAvailableStock = (product, variantIndex) => {
...
const calculateBundleStock = (bundle) => {
...
export const bundleService = {
    normalizeBundleItems: async (bundleItems, currentProductId) => {
        ...
        return normalizedItems.map((item) => {
            ...
            const resolvedVariantIndex = item.variantIndex !== undefined ? item.variantIndex : product.variants.length === 1 ? 0 : undefined;
    ...
    buildBundleOrderItems: async (bundleProduct) => {
    ...
    recalculateBundleStocksForProductIds: async (productIds, session) => {
        const normalizedIds = [...new Set(productIds.map((productId) => productId.toString()))].map((productId) => new Types.ObjectId(productId));
        ...
        for (const bundle of bundles) {
            const bundleItems = bundle.bundleItems.filter((item) => Boolean(item.product && typeof item.product === 'object' && 'variants' in item.product));
