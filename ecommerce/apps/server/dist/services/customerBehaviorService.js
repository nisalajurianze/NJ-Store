...
const toStringValue = (value, maxLength) => {
...
const buildEventDocument = (event, userId) => {
...
const fetchWantedProductDemand = async (limit) => CustomerBehaviorEvent.aggregate([
    {
        $match: {
...
const fetchProductCardsByDemand = async (limit) => {
...
export const customerBehaviorService = {
    recordEvents: async (events, userId) => {
    ...
    getWantedProductCards: async (limit = 4) => cacheService.rememberVersioned(cacheNamespaces.customerBehavior, `wanted-products:${limit}`, WANTED_PRODUCTS_CACHE_TTL_SECONDS, () => fetchProductCardsByDemand(limit)),
    getCustomerMining: async (windowDays = CUSTOMER_MINING_WINDOW_DAYS) => cacheService.rememberVersioned(cacheNamespaces.customerBehavior, `mining:${windowDays}`, CUSTOMER_MINING_CACHE_TTL_SECONDS, async () => {
        ...
        const siteEngagementScore = Math.min(100, Math.round(averagePageViewsPerVisitor * 12 +
