...
const getNotificationFingerprint = (item) => [item.id, item.count, item.createdAt ?? 'undated'].join(':');
const getUnreadItems = async (items, userId) => {
...
export const notificationAdminService = {
    getCenter: async (permissions, userId) => {
        const canReadOrders = hasPermission(permissions, 'order:read');
        const canReadProducts = hasPermission(permissions, 'product:read');
        const [pendingOrders, receiptUploads, pendingReturns, lowStockVariants, pendingQuestions, pendingReviews] = await Promise.all([
            canReadOrders
                ? Promise.all([
                    Order.countDocuments({ deletedAt: null, isQuotation: false, status: 'pending' }),
                    Order.findOne({ deletedAt: null, isQuotation: false, status: 'pending' }).sort({ createdAt: -1 }).select('createdAt').lean()
                ]).then(([count, latest]) => ({ count, createdAt: toIsoDate(latest?.createdAt) }))
                : Promise.resolve({ count: 0 }),
            canReadOrders
                ? Promise.all([
                    Order.countDocuments({ deletedAt: null, paymentStatus: 'receipt_uploaded' }),
                    Order.findOne({ deletedAt: null, paymentStatus: 'receipt_uploaded' }).sort({ updatedAt: -1 }).select('updatedAt').lean()
                ]).then(([count, latest]) => ({ count, createdAt: toIsoDate(latest?.updatedAt) }))
                : Promise.resolve({ count: 0 }),
            canReadOrders
                ? Promise.all([
                    ReturnRequest.countDocuments({ status: 'pending' }),
                    ReturnRequest.findOne({ status: 'pending' }).sort({ createdAt: -1 }).select('createdAt').lean()
                ]).then(([count, latest]) => ({ count, createdAt: toIsoDate(latest?.createdAt) }))
                : Promise.resolve({ count: 0 }),
            canReadProducts
            ...
            canReadProducts
                ? Promise.all([
                    ...
                    ProductQuestion.findOne({ status: 'pending' }).sort({ createdAt: -1 }).select('createdAt').lean()
                ]).then(([count, latest]) => ({ count, createdAt: toIsoDate(latest?.createdAt) }))
                : Promise.resolve({ count: 0 }),
            canReadProducts
                ? Promise.all([
                    Review.countDocuments({ isApproved: false }),
                    Review.findOne({ isApproved: false }).sort({ createdAt: -1 }).select('createdAt').lean()
                ]).then(([count, latest]) => ({ count, createdAt: toIsoDate(latest?.createdAt) }))
                : Promise.resolve({ count: 0 })
        ]);
        const rawItems = [
        ...
        const items = rawItems.filter((item) => Boolean(item));
    ...
    markViewed: async (permissions, userId, notificationId) => {
        ...
        if (activeItem) {
            await AdminNotificationView.updateOne({ user: userId, notificationId }, {
                $set: {
