...
const HOME_HERO_KEY = 'home-hero';
const BANNER_CACHE_TTL_SECONDS = 10 * 60;
const BANNER_QUERY_TIMEOUT_MS = 5_000;
const HOME_AD_SLOT_KEYS = ['slot-1', 'slot-2', 'slot-3'];
const BANNER_SELECT = [
    'key',
    'campaignLabel',
    'title',
...
const hasLegacyMedia = (value) => Boolean(value && typeof value === 'object' && 'url' in value && 'publicId' in value);
const migrateLegacyMediaItems = (entry) => {
...
const migrateLegacyHomeHeroBannerFields = async () => {
    ...
    for (const legacyBanner of legacyBanners) {
        ...
        if (Array.isArray(legacyBanner.adSlots)) {
            set.adSlots = legacyBanner.adSlots.map((slot) => slot && typeof slot === 'object' ? migrateLegacyMediaItems(slot) : slot);
        }
        if (legacyBanner.featurePromo && typeof legacyBanner.featurePromo === 'object') {
            set.featurePromo = migrateLegacyMediaItems(legacyBanner.featurePromo);
        }
        await Banner.collection.updateOne({ _id: legacyBanner._id }, {
...
let legacyHomeHeroBannerMigrationPromise = null;
const ensureLegacyHomeHeroBannerMigration = () => {
...
const normalizeAdSlots = (input) => HOME_AD_SLOT_KEYS.map((slotKey) => {
...
const normalizeFeaturePromo = (input) => {
...
const normalizeShowcaseFeatureGroups = (input, showcaseProductIds) => showcaseProductIds.flatMap((productId) => {
...
const defaultHomeHeroBanner = () => ({
    id: HOME_HERO_KEY,
    key: HOME_HERO_KEY,
    campaignLabel: 'NJ Store',
    title: 'Electronics curated for premium everyday performance.',
    subtitle: 'Flagship phones, productivity laptops, dependable printers, and refined accessories with quotation-first checkout.',
...
export const bannerService = {
    ...
    upsertHomeHeroBanner: async (payload) => {
        await ensureLegacyHomeHeroBannerMigration();
        ...
        banner.set('showcaseProducts', showcaseProductIds.map((id) => new Types.ObjectId(id)));
