...
export const buildProductVersionSnapshot = (product) => ({
    name: product.name,
...
const serializeProductVersion = (version) => ({
...
export const productVersionService = {
    recordVersion: async (payload) => {
        ...
        while (remainingAttempts > 0) {
            try {
                ...
                const isDuplicateVersionError = typeof error === 'object' &&
                    error !== null &&
                    'code' in error &&
    ...
    listVersions: async (productId, limit = 5) => {
        const versions = await ProductVersion.find({ product: productId })
            .sort({ version: -1 })
            .limit(limit)
            .populate('updatedBy', 'name email')
            .lean();
        return versions.map((version) => serializeProductVersion(version));
    },
    getVersionById: async (productId, versionId) => {
        const version = await ProductVersion.findOne({
            ...
            .populate('updatedBy', 'name email')
            .lean();
        return version ? serializeProductVersion(version) : null;
    },
    deleteVersionsForProduct: async (productId) => {
