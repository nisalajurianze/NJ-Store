...
const ensureDirectory = async (root, folder) => {
    const target = path.join(root, folder);
    await fs.mkdir(target, { recursive: true });
    return target;
};
const detectFile = async (file, allowMimeFallback) => {
...
const resolveCloudinaryResourceType = (resourceType, mime) => resourceType === 'auto'
    ? (mime.startsWith('image/') ? 'image' : 'raw')
    : (resourceType ?? 'image');
const encodePrivateCloudinaryPublicId = (resourceType, format, publicId) => `${PRIVATE_CLOUDINARY_PREFIX}${resourceType}:${format}:${publicId}`;
const parsePrivateCloudinaryPublicId = (publicId) => {
...
const uploadToCloudinary = async (file, folder, resourceType, options) => new Promise((resolve, reject) => {
    if (!file.buffer) {
...
const writeLocally = async (visibility, folder, fileName, file) => {
    let lastError;
    for (const root of localUploadRoots[visibility]) {
        try {
            ...
            lastError = error;
            logger.warn(`upload.local_write_failed visibility=${visibility} root=${root} folder=${folder} reason=${error instanceof Error ? error.message : 'unknown'}`);
        }
    }
    throw new AppError(`Unable to store ${visibility === 'private' ? 'private' : 'public'} asset locally${lastError instanceof Error ? `: ${lastError.message}` : ''}`, 500);
};
/**
 * Uploads a file to Cloudinary when configured, or to a local public uploads folder otherwise.
 */
export const uploadBuffer = async ({ file, folder, baseUrl, allowedMimeTypes, alt, resourceType = 'image', allowMimeFallback = false, visibility = 'public' }) => {
    try {
        ...
        if (isCloudinaryConfigured) {
            try {
                ...
                logger.warn(`upload.cloudinary_failed visibility=${visibility} folder=${folder} reason=${error instanceof Error ? error.message : 'unknown'}`);
            }
        }
...
export const uploadGeneratedBuffer = async ({ buffer, fileName, mimeType, folder, baseUrl, alt, resourceType = 'raw' }) => uploadBuffer({
    file: {
        fieldname: folder,
...
export const isLocalAsset = (publicId) => publicId.startsWith('local:') || publicId.startsWith('private:');
export const resolveAssetDeliveryUrl = (asset) => {
...
export const resolveLocalAssetPath = (publicId) => {
