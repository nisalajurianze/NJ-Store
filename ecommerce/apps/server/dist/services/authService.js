...
const cookieSameSite = env.NODE_ENV === 'production' ? 'none' : 'strict';
...
const isCookieDomainMatch = (requestHost, cookieDomain) => {
...
export const resolveRefreshCookieDomain = (requestHost, configuredDomain = env.COOKIE_DOMAIN) => {
...
export const shouldPartitionRefreshCookie = (domain, nodeEnv = env.NODE_ENV) => nodeEnv === 'production' && !domain;
const buildRefreshCookieBaseOptions = (requestHost) => {
...
const createAccessToken = (user, sessionId) => jwt.sign({
    ...
    sessionId
}, env.JWT_ACCESS_SECRET, { expiresIn: env.JWT_ACCESS_EXPIRES });
const createRefreshToken = (user, sessionId, rememberMe) => jwt.sign({
    id: user._id.toString(),
    email: user.email,
    role: user.role,
    language: user.language,
    ...
    expiresIn: (rememberMe ? '30d' : env.JWT_REFRESH_EXPIRES)
});
const buildAuthTokens = async (user, rememberMe, meta) => {
...
const buildVerificationUrl = (token) => `${env.CLIENT_URL}/auth/verify-email?token=${token}`;
const buildResetUrl = (token) => `${env.CLIENT_URL}/auth/reset-password?token=${token}`;
const createEmailVerification = async (userId, name, email) => {
...
const createPasswordReset = async (userId, name, email) => {
    const rawToken = createRandomToken();
    await User.findByIdAndUpdate(userId, {
...
const createAuthPayload = async (user, sessionId, tokens) => ({
...
export const authService = {
    getRefreshCookieOptions: (rememberMe, requestHost) => ({
        ...buildRefreshCookieBaseOptions(requestHost),
        maxAge: rememberMe ? rememberMeMs : refreshTokenMs
    }),
    getRefreshCookieClearOptions: (requestHost) => buildRefreshCookieBaseOptions(requestHost),
    register: async (payload, meta) => {
    ...
    login: async (email, password, rememberMe, meta) => {
    ...
    loginWithGoogle: async (credential, rememberMe, workspaceAccess, meta) => {
        let email = '';
        let name = '';
        let googleId = '';
        let picture = '';
    ...
    refresh: async (rawRefreshToken, meta) => {
        let decoded = null;
        try {
    ...
    logout: async (rawRefreshToken) => {
        ...
        try {
            ...
            const session = await RefreshSession.findOneAndUpdate({ tokenHash: hashValue(rawRefreshToken) }, { revokedAt: new Date() }, { new: false })
                .select('sessionId')
                .lean();
            await invalidateCachedSession(session?.sessionId);
    ...
    listSessions: async (userId, currentSessionId) => getSessions(new Types.ObjectId(userId), currentSessionId),
    listLoyaltyTransactions: async (userId) => {
    ...
    updateProfile: async (userId, payload) => {
        ...
        if (payload.shopPreferences) {
            if (payload.shopPreferences.myFilters === null) {
                ...
                $set['shopPreferences.myFilters'] = {
                    params: Object.fromEntries(Object.entries(payload.shopPreferences.myFilters.params).filter((entry) => typeof entry[1] === 'string' && entry[1].length > 0)),
        ...
        const user = await User.findByIdAndUpdate(userId, Object.keys(updateDocument).length ? updateDocument : {}, { new: true });
        if (!user) {
            throw new AppError('User not found', 404);
        }
        await invalidateCachedUser(user._id);
    ...
    cleanupExpiredSecurityFields: async () => {
        const now = new Date();
