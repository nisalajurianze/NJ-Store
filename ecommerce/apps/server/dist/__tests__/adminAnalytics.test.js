...
const registerAnalyticsAdmin = async (email, permissions = ['order:read', 'product:read', 'user:read']) => {
    await request(app)
...
const createCustomerAt = async (email, createdAt) => {
    ...
    await User.collection.updateOne({ _id: customer._id }, {
        $set: {
            createdAt,
            updatedAt: createdAt
        }
...
describe('Admin analytics', () => {
    it('counts total customers across the full base when building the 30-day growth series', async () => {
        ...
        expect(res.body.data.kpis.find((kpi) => kpi.label === 'New Customers')?.value).toBe(1);
    }, TEST_TIMEOUT);
    it('refreshes analytics immediately after customer registration invalidates the cache', async () => {
        ...
        expect(second.body.data.kpis.find((kpi) => kpi.label === 'New Customers')?.value).toBe(1);
    }, TEST_TIMEOUT);
    it('tracks the live order queue in status breakdown while keeping revenue metrics paid-only', async () => {
        ...
        const res = await request(app)
            ...
            .set('Authorization', `Bearer ${token}`)
            .expect(200);
        const statusBreakdown = Object.fromEntries(res.body.data.statusBreakdown.map((entry) => [entry.status, entry.count]));
        ...
        expect(res.body.data.kpis.find((kpi) => kpi.label === 'Total Orders')?.value).toBe(1);
        expect(res.body.data.kpis.find((kpi) => kpi.label === 'Total Revenue')?.value).toBe(2500);
    }, TEST_TIMEOUT);
    it('includes manual outgoing expenses in the sales analysis snapshots and period totals', async () => {
        ...
        const dailyPoint = res.body.data.dailySales.find((point) => point.period === currentDayKey);
        const monthlyPoint = res.body.data.monthlySales.find((point) => point.period === currentMonthKey);
        const yearlyPoint = res.body.data.yearlySales.find((point) => point.period === currentYearKey);
    ...
    it('creates, updates, and deletes manual expenses through the admin sales analysis routes', async () => {
        ...
        const createdExpense = await request(app)
            ...
            .expect(201);
        const expenseId = createdExpense.body.data.id;
        await request(app)
        ...
        const afterUpdate = await request(app)
            ...
            .set('Authorization', `Bearer ${token}`)
            .expect(200);
        expect(afterUpdate.body.data.expenses).toEqual(expect.arrayContaining([
    ...
    it('builds cohort retention rows from first-order months and repeat purchase activity', async () => {
        ...
        const olderCohort = res.body.data.retentionCohorts.find((cohort) => cohort.cohortMonth === olderCohortKey);
        const currentCohort = res.body.data.retentionCohorts.find((cohort) => cohort.cohortMonth === currentCohortKey);
        expect(olderCohort).toMatchObject({
            cohortMonth: olderCohortKey,
            cohortSize: 2
        });
        expect(olderCohort.retention).toEqual(expect.arrayContaining([
        ...
        expect(currentCohort.retention).toEqual(expect.arrayContaining([
    ...
    it('exports the sales analysis report as a PDF', async () => {
    ...
    it('returns RFM segments for recent, at-risk, new, and dormant paid customers', async () => {
        ...
        const res = await request(app)
            ...
            .set('Authorization', `Bearer ${token}`)
            .expect(200);
        const segmentCounts = Object.fromEntries(res.body.data.rfmSegments.map((entry) => [entry.key, entry.customerCount]));
        const customerSegments = Object.fromEntries(res.body.data.rfmCustomers.map((entry) => [entry.email, entry.segmentKey]));
    ...
    it('buckets paid revenue by payment confirmation time instead of order creation time', async () => {
        ...
        const analyticsRes = await request(app)
            ...
            .set('Authorization', `Bearer ${token}`)
            .expect(200);
        expect(analyticsRes.body.data.kpis.find((kpi) => kpi.label === 'Total Revenue')?.value).toBe(2200);
        expect(analyticsRes.body.data.kpis.find((kpi) => kpi.label === 'Total Orders')?.value).toBe(1);
        const salesAnalysisRes = await request(app)
            .get('/api/v1/admin/sales-analysis')
            .set('Authorization', `Bearer ${token}`)
            .expect(200);
        const insideDayPoint = salesAnalysisRes.body.data.dailySales.find((point) => point.period === insideWindow.format('YYYY-MM-DD'));
        expect(insideDayPoint).toMatchObject({
            revenue: 2200,
            orderCount: 1
        });
    }, TEST_TIMEOUT);
    it('filters analytics by the requested period and returns funnel plus district coverage for that window', async () => {
        ...
        expect(res.body.data.kpis.find((kpi) => kpi.label === 'Total Revenue')?.value).toBe(4800);
        expect(res.body.data.kpis.find((kpi) => kpi.label === 'Total Orders')?.value).toBe(1);
