...
const createFile = (name, mimeType, buffer) => ({
    fieldname: 'receipt',
    originalname: name,
    encoding: '7bit',
    mimetype: mimeType,
...
describe('uploadService private Cloudinary assets', () => {
    ...
    it('encodes private Cloudinary receipts and resolves a signed delivery URL', async () => {
        ...
        expect(uploadStreamMock).toHaveBeenCalledWith(expect.objectContaining({
            folder: 'receipts',
            resource_type: 'image',
            type: 'private'
        }), expect.any(Function));
        ...
        expect(privateDownloadUrlMock).toHaveBeenCalledWith('receipts/receipt', 'png', expect.objectContaining({
            resource_type: 'image',
            type: 'private'
        }));
        await removeAsset(asset.publicId);
        expect(destroyMock).toHaveBeenCalledWith('receipts/receipt', expect.objectContaining({
            invalidate: true,
            resource_type: 'image',
            type: 'private'
        }));
    ...
    it('removes disk-backed multer temp files after upload completes', async () => {
        ...
        expect(uploadMock).toHaveBeenCalledWith(tempPath, expect.objectContaining({
            folder: 'receipts',
            resource_type: 'image'
        }));
