...
describe('Admin product enhancements', () => {
    it('keeps future-scheduled products hidden from the public API until publish time and records versions', async () => {
        ...
        await productService.updateProduct(createdProduct.id, { publishAt: new Date(Date.now() - 60 * 1000).toISOString() }, { commitMessage: 'Published scheduled product' });
    ...
    it('bulk-adjusts prices, records a new version, and restores an older snapshot on demand', async () => {
        const category = await Category.create({
            name: 'Accessories',
            slug: 'accessories',
