...
const commaSeparatedObjectIdsSchema = z.preprocess((value) => {
...
const urlOrEmptySchema = z.preprocess((value) => (typeof value === 'string' ? value.trim() : value), z.string().url('Enter a valid URL').or(z.literal('')));
...
const optionalShortTextSchema = (max) => z.preprocess((value) => (typeof value === 'string' ? value.trim() : value), z.string().max(max).or(z.literal('')).optional());
