...
import type { AnalyticsSummaryDto, OrderDto, SalesAnalysisDto, SiteConfigDto } from '@njstore/types';
...
const fetchImageBuffer = async (url: string | undefined): Promise<Buffer | null> => {
...
const resolveDocumentLogo = async (siteConfig: SiteConfigDto): Promise<Buffer | null> =>
  (await fetchImageBuffer(siteConfig.storeLogoLight?.url)) ??
  (await fetchImageBuffer(siteConfig.storeLogo?.url)) ??
  (await fetchImageBuffer(siteConfig.storeLogoDark?.url));

const drawWordmark = (doc: PDFKit.PDFDocument, siteConfig: SiteConfigDto, logoBuffer: Buffer | null): void => {
  if (logoBuffer) {
    try {
...
const drawOrderTableHeader = (doc: PDFKit.PDFDocument, y: number): number => {
 ...
 */
export const generateOrderPdfBuffer = async (
  order: OrderDto,
  siteConfig: SiteConfigDto,
  documentType: 'QUOTATION' | 'INVOICE',
  ...
  return new Promise((resolve, reject) => {
    ...
    drawInfoCard(
      doc,
      'Document Details',
      [`Payment: ${paymentLabel}`, `Payment status: ${paymentStatusLabel}`, `Order status: ${orderStatusLabel}`, `Fulfilment: ${fulfilmentLabel}`],
      344,
      146,
      203,
      96
    ...
    const summaryLines: Array<[string, string]> = [
      ...
      ...((order.loyaltyDiscount ?? 0) > 0
        ? ([['Loyalty points', `-${formatCurrency(order.loyaltyDiscount ?? 0)}`]] as Array<[string, string]>)
        : []),
      ...(taxLabel ? ([[taxLabel, formatCurrency(order.taxAmount ?? 0)]] as Array<[string, string]>) : [])
    ];
    ...
    doc
      ...
      .text(
        documentType === 'QUOTATION'
          ...
          : 'Thank you for shopping with NJ Store.',
        PAGE_MARGIN,
        footerY,
        { width: 260 }
      );
    doc.text(
      order.paymentMethod === 'cash_on_delivery'
        ...
        : `Bank transfer: ${siteConfig.bankTransferDetails.accountName} | ${siteConfig.bankTransferDetails.bankName} | ${siteConfig.bankTransferDetails.accountNumber}`,
      PAGE_MARGIN,
      footerY + 18,
      { width: CONTENT_WIDTH }
    );
...
const addSectionHeading = (doc: PDFKit.PDFDocument, title: string, y: number): number => {
...
const ensurePageSpace = (doc: PDFKit.PDFDocument, y: number, minHeight = 72): number => {
  if (y + minHeight <= 752) {
    return y;
  }
...
export const generateAnalyticsPdfBuffer = async (
  ...
  new Promise((resolve, reject) => {
    ...
    doc
      ...
      .fillColor('#6B7280')
      .text(
        `Dashboard report generated from the admin analytics export on ${formatDate(new Date())}.`,
        48,
        784,
        { align: 'left' }
      );
...
export const generateSalesAnalysisPdfBuffer = async (
  salesAnalysis: SalesAnalysisDto,
  siteConfig: SiteConfigDto
): Promise<Buffer> =>
  new Promise((resolve, reject) => {
    const chunks: Buffer[] = [];
    ...
    const projectedRevenue = Array.from({ length: 7 }, (_, index) => Math.max(0, intercept + slope * (pointCount + index))).reduce(
      (sum, value) => sum + value,
      0
    );
    ...
    doc.text(
      salesAnalysis.strongestMonth ? formatCurrency(salesAnalysis.strongestMonth.revenue) : 'No monthly revenue history yet.',
      300,
      y + 34,
      { width: 220 }
    );
    ...
    if (!salesAnalysis.retentionCohorts.length) {
      ...
      salesAnalysis.retentionCohorts.slice(0, 6).forEach((cohort) => {
        ...
        doc.text(
          `Month 1 retention: ${monthOneRate === null || monthOneRate === undefined ? 'N/A' : `${Math.round(monthOneRate * 100)}%`}`,
          300,
          y,
          { width: 220 }
        );
