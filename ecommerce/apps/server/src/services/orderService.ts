...
import type { AddressDto, AdminOrderDto, OrderDto, PaginatedResult } from '@njstore/types';
...
interface AdminManualOrderInput {
  customerId?: string;
  customerName: string;
...
interface QuotationFulfilmentInput {
  paymentMethod?: 'bank_transfer' | 'cash_on_delivery';
  type?: 'delivery' | 'pickup';
  addressId?: string;
  shippingAddress?: AddressDto;
...
type NotificationChannelKey = 'emailEnabled' | 'smsEnabled';
type NotificationPreferenceKey = 'quotationReady' | 'orderConfirmed' | 'orderShipped' | 'receiptRejected' | 'lowStockAlert';

const isEmailAddress = (value: string): boolean => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value);

const resolveLowStockAlertEmail = (): string | null => {
...
const sendLowStockAlertSafely = async (productName: string, sku: string, stock: number): Promise<void> => {
  ...
  try {
    ...
    logger.warn(
      `order.low_stock_email_failed product=${productName} sku=${sku} reason=${error instanceof Error ? error.message : 'unknown'}`
    );
  }
};

type DocumentCounterDocument = {
  _id: string;
  sequence: number;
};

const documentCounterSchema = new mongoose.Schema<DocumentCounterDocument>(
  {
...
const DocumentCounter =
  (mongoose.models.DocumentCounter as mongoose.Model<DocumentCounterDocument> | undefined) ??
  mongoose.model<DocumentCounterDocument>('DocumentCounter', documentCounterSchema);

const reserveDocumentNumber = async (
  prefix: 'ORD' | 'QTN',
  session?: mongoose.ClientSession
): Promise<string> => {
  const dateKey = dayjs().format('YYYYMMDD');
  const counterId = `${prefix}-${dateKey}`;
  const counter = await DocumentCounter.findByIdAndUpdate(
    counterId,
    {
      $inc: { sequence: 1 },
...
const addBusinessDays = (startDate: Date, range: string): Date => {
...
const getBaseUrl = (baseUrl: string): string => baseUrl.replace(/\/$/, '');

const invalidateAnalyticsCache = async (): Promise<void> => {
  await cacheService.bumpNamespace(cacheNamespaces.analytics);
};

const resolveShipping = async (address: AddressDto | undefined, subtotal: number) => {
  const config = await siteConfigService.getOrCreateDocument();
  if (!address) {
    return { fee: 0, days: '0', estimatedDeliveryDate: undefined as Date | undefined };
  }
...
const resolveQuotationFulfilment = async (
  userId: string,
  order: {
    type: 'delivery' | 'pickup';
    shippingAddress?: AddressDto;
  ...
  payload: QuotationFulfilmentInput
): Promise<{
  type: 'delivery' | 'pickup';
  shippingAddress?: AddressDto;
  pickupSlot?: string;
...
const buildOrderItems = async (items: CheckoutItemInput[], session?: mongoose.ClientSession): Promise<ResolvedOrderItem[]> => {
  if (!items.length) {
...
const resolveLoyaltyRedemption = (
...
const restoreLoyaltyRedemption = async (
  order: {
    _id: Types.ObjectId;
    orderNumber: string;
...
): { taxAmount: number; taxLabel: string; taxRate: number } => {
...
const isNotificationChannelEnabled = (
  settings:
    | {
...
): boolean => settings?.[key]?.[channel] ?? (channel === 'emailEnabled');

const resolveCustomerNotificationPhone = (
  shippingAddress: AddressDto | undefined,
  userPhone: string | undefined | null
): string | null => shippingAddress?.phone?.trim() || userPhone?.trim() || null;

const collectAffectedProductIdsFromOrderItems = (items: ResolvedOrderItem[]): string[] =>
  items.flatMap((item) => {
    const productId = resolveStoredOrderItemProductId(item).trim();
    const bundleProductIds =
      item.bundleItems?.flatMap((bundleItem) => {
...
const orderStatusLabel = (status: 'pending' | 'processing' | 'shipped' | 'delivered' | 'cancelled'): string =>
  ({
    pending: 'Pending',
...
const captureProductStockSnapshot = async (
...
const decrementInventory = async (
  ...
  for (const item of items) {
    ...
    if (item.bundleItems?.length) {
      const bundle = await Product.findByIdAndUpdate(
        item.productId,
...
const restoreInventory = async (
  items: ResolvedOrderItem[],
  session?: mongoose.ClientSession
): Promise<Array<{ before: ProductStockSnapshot; after: ProductStockSnapshot }>> => {
  const affectedProductIds = new Set<string>();
  const stockTransitions: Array<{ before: ProductStockSnapshot; after: ProductStockSnapshot }> = [];
...
const restoreCommittedOrderEffects = async (
  order: {
    _id: Types.ObjectId;
    orderNumber: string;
    isQuotation: boolean;
...
const ensureOrderOwnership = (orderUserId: Types.ObjectId, userId?: string): void => {
  if (userId && orderUserId.toString() !== userId) {
    throw new AppError('Order not found', 404);
  }
};

function resolveStoredOrderItemProductId(item: { productId?: string; product?: Types.ObjectId | string }): string {
  if (item.productId) {
...
const getReceiptTimestamp = (order: { updatedAt?: Date; createdAt?: Date }): Date => order.updatedAt ?? order.createdAt ?? new Date();

const getReceiptSnapshot = (order: {
  receipts?: ReceiptAssetLike[] | null;
  receipt?: { url: string; publicId: string; alt?: string } | null;
  updatedAt?: Date;
  createdAt?: Date;
}): ReceiptAssetLike[] => {
...
const migrateLegacyReceiptIfNeeded = async (order: any, session?: mongoose.ClientSession): Promise<void> => {
...
const assertEditableReceiptState = (order: { status: string; isQuotation: boolean; paymentStatus: string }): void => {
...
const normalizeComparableText = (value?: string | null): string => value?.trim().replace(/\s+/g, ' ').toLowerCase() ?? '';

const areEquivalentAddresses = (left?: AddressDto, right?: AddressDto): boolean => {
  if (!left && !right) {
    return true;
  }
...
const combineUniqueText = (...values: Array<string | undefined | null>): string | undefined => {
  const unique = values
...
const resolveManualOrderCustomer = async (
  payload: AdminManualOrderInput,
  session: mongoose.ClientSession
...
export const orderService = {
  adminCreateOrder: async (
    ...
    let customerForDocument: UserDocument | null = null;
      ...
      await session.withTransaction(async () => {
        ...
        const resolvedShipping =
          orderType === 'delivery'
            ...
            : { fee: 0, days: '0', estimatedDeliveryDate: undefined as Date | undefined };
    ...
    if (shouldGenerateInvoice) {
      try {
        ...
        const invoiceBuffer = await generateOrderPdfBuffer(
          serializeOrder(orderToReturn.toObject()),
          siteConfig,
          'INVOICE',
        ...
        logger.warn(
          `order.manual_invoice_generation_failed order=${orderToReturn._id.toString()} reason=${error instanceof Error ? error.message : 'unknown'}`
        );
      }
    }
    ...
    try {
      ...
      logger.warn(
        `order.manual_notification_failed order=${orderToReturn._id.toString()} reason=${error instanceof Error ? error.message : 'unknown'}`
      );
    }
  ...
  createQuotation: async (userId: string, payload: CreateQuotationInput, baseUrl: string): Promise<OrderDto> => {
    const user = await User.findById(userId);
    if (!user) {
    ...
    const loyaltyRedemption = resolveLoyaltyRedemption(
      payload.loyaltyPointsToRedeem,
      user.loyaltyPoints,
    ...
    try {
      ...
      logger.warn(
        `order.quotation_document_failed order=${order._id.toString()} reason=${error instanceof Error ? error.message : 'unknown'}`
      );
    }
    ...
    if (isNotificationChannelEnabled(config.notificationSettings, 'quotationReady', 'emailEnabled') && quotationPdfReady && order.quotationPdf?.url) {
      void Promise.resolve(
        emailService.sendQuotation(
          ...
          user.email,
          quotationUrl,
          order.quotationPdf.url,
          formatDate(order.quotationExpiry ?? new Date())
        )
      ).catch((error) => {
        logger.warn(
          `order.quotation_email_failed order=${order._id.toString()} reason=${error instanceof Error ? error.message : 'unknown'}`
        );
      });
    }
    ...
    if (isNotificationChannelEnabled(config.notificationSettings, 'quotationReady', 'smsEnabled')) {
      ...
      if (customerPhone) {
        void Promise.resolve(smsService.sendQuotationReady(customerPhone, quotationNumber, quotationUrl)).catch((error) => {
          logger.warn(
            `order.quotation_sms_failed order=${order._id.toString()} reason=${error instanceof Error ? error.message : 'unknown'}`
          );
        });
      }
    }
  ...
  getQuotationByToken: async (token: string, userId: string): Promise<OrderDto> => {
  ...
  confirmQuotation: async (
    ...
    try {
      await session.withTransaction(async () => {
        ...
        const recalculatedItems = await buildOrderItems(
        ...
        const fulfilment = await resolveQuotationFulfilment(
          user._id.toString(),
          {
            type: order.type,
            shippingAddress: order.shippingAddress as AddressDto | undefined,
        ...
        const shipping =
          fulfilment.type === 'delivery'
            ...
            : { fee: 0, days: '0', estimatedDeliveryDate: undefined as Date | undefined };
        const couponResult = await getCouponOutcome(
          user._id.toString(),
          user.email,
          order.couponCode,
          subtotal,
        ...
        const loyaltyRedemption = resolveLoyaltyRedemption(
          payload.loyaltyPointsToRedeem ?? order.loyaltyPointsRedeemed,
          user.loyaltyPoints,
          couponResult.finalTotal
        );
        const taxableAmount = Math.max(couponResult.finalTotal - loyaltyRedemption.discount, 0);
        ...
        order.set('items', recalculatedItems as any);
    ...
    if (isNotificationChannelEnabled(notificationConfig.notificationSettings, 'orderConfirmed', 'emailEnabled')) {
      void Promise.resolve(emailService.sendOrderConfirmation(user.name, user.email, orderToReturn.orderNumber)).catch((error) => {
        logger.warn(
          `order.confirmation_email_failed order=${orderToReturn._id.toString()} reason=${error instanceof Error ? error.message : 'unknown'}`
        );
      });
    }

    if (isNotificationChannelEnabled(notificationConfig.notificationSettings, 'orderConfirmed', 'smsEnabled')) {
      const customerPhone = resolveCustomerNotificationPhone(orderToReturn.shippingAddress as AddressDto | undefined, user.phone);
      if (customerPhone) {
        void Promise.resolve(smsService.sendOrderConfirmation(customerPhone, orderToReturn.orderNumber)).catch((error) => {
          logger.warn(
            `order.confirmation_sms_failed order=${orderToReturn._id.toString()} reason=${error instanceof Error ? error.message : 'unknown'}`
          );
        });
      }
    }
    ...
    try {
      ...
      logger.warn(
        `order.confirmation_notification_failed order=${orderToReturn._id.toString()} reason=${error instanceof Error ? error.message : 'unknown'}`
      );
    }
  ...
  listOrders: async (
    ...
    page = 1,
    limit = 10,
    sortBy: 'createdAt' | 'activity' = 'createdAt'
  ): Promise<PaginatedResult<OrderDto>> => {
    const orderSort = sortBy === 'activity' ? { updatedAt: -1 as const, createdAt: -1 as const } : { createdAt: -1 as const };
  ...
  getOrderById: async (orderId: string, userId?: string): Promise<OrderDto> => {
  ...
  cancelOrder: async (orderId: string, userId: string, reason?: string): Promise<OrderDto> => {
    const session = await mongoose.startSession();
    let orderToReturn: any;
    let inventoryTransitions: Array<{ before: ProductStockSnapshot; after: ProductStockSnapshot }> = [];
    ...
    try {
      await session.withTransaction(async () => {
        ...
        inventoryTransitions = await restoreCommittedOrderEffects(
          order as unknown as {
            _id: Types.ObjectId;
            orderNumber: string;
            isQuotation: boolean;
    ...
    await invalidateInventoryDerivedCaches();
    await inventoryBroadcastService.broadcastProductStockUpdates(collectAffectedProductIdsFromOrderItems(orderToReturn.items as ResolvedOrderItem[]));
  ...
  removeReceipt: async (orderId: string, userId: string, receiptId: string): Promise<OrderDto> => {
    const order = await Order.findById(orderId);
    if (!order || order.deletedAt) {
      throw new AppError('Order not found', 404);
    }
  ...
  getInvoiceAsset: async (orderId: string, userId?: string): Promise<{ url: string; publicId: string; alt?: string }> => {
    const order = await Order.findById(orderId);
    if (!order || order.deletedAt) {
      throw new AppError('Order not found', 404);
    }
  ...
  getQuotationAsset: async (orderId: string, userId?: string): Promise<{ url: string; publicId: string; alt?: string }> => {
    const order = await Order.findById(orderId);
    if (!order || order.deletedAt) {
      throw new AppError('Order not found', 404);
    }
  ...
  listAllOrders: async (filters: {
    ...
    const query: Record<string, unknown> = { deletedAt: null, isQuotation: false };
    if (filters.status) {
  ...
  adminUpdateOrder: async (
    ...
    if (payload.status === 'cancelled') {
      ...
      try {
        await session.withTransaction(async () => {
          ...
          inventoryTransitions = await restoreCommittedOrderEffects(
            transactionalOrder as unknown as {
              _id: Types.ObjectId;
              orderNumber: string;
              isQuotation: boolean;
              couponCode?: string;
            ...
            transactionalOrder.user.toString(),
            session
          );
      ...
      logger.info(
        `order.admin_updated order=${orderToReturn._id.toString()} actor=${actorName} status=${orderToReturn.status} payment=${orderToReturn.paymentStatus}`
      );
      await invalidateInventoryDerivedCaches();
      await inventoryBroadcastService.broadcastProductStockUpdates(collectAffectedProductIdsFromOrderItems(orderToReturn.items as ResolvedOrderItem[]));
      for (const transition of inventoryTransitions) {
    ...
    const session = await mongoose.startSession();
    let orderToReturn: any;
    let receiptRejectedEmail: { name: string; email: string; phone?: string; orderNumber: string; reason: string } | null = null;
    let shouldGenerateInvoice = false;
    let customerUpdateNotification:
      | {
          userId: string;
          title: string;
          body: string;
      ...
      await session.withTransaction(async () => {
        ...
        if (payload.paymentStatus === 'rejected') {
          ...
          receiptRejectedEmail = {
            ...
            phone: resolveCustomerNotificationPhone(transactionalOrder.shippingAddress as AddressDto | undefined, transactionalUser.phone),
        ...
        const customerUpdateParts: string[] = [];
    ...
    if (receiptRejectedEmail) {
      ...
      if (isNotificationChannelEnabled(notificationConfig.notificationSettings, 'receiptRejected', 'emailEnabled')) {
        await emailService.sendReceiptRejected(
          receiptRejectedEmail.name,
          receiptRejectedEmail.email,
          receiptRejectedEmail.orderNumber,
          receiptRejectedEmail.reason
        );
      }
    ...
    if (shouldGenerateInvoice) {
      try {
        ...
        logger.warn(
          `order.invoice_generation_failed order=${orderToReturn._id.toString()} reason=${error instanceof Error ? error.message : 'unknown'}`
        );
      }
    }
  ...
  sendShippingNotification: async (
    orderId: string,
    actorName: string
  ): Promise<{ orderNumber: string; recipient: string; trackingNumber: string }> => {
    const order = await Order.findById(orderId);
    if (!order || order.deletedAt) {
      throw new AppError('Order not found', 404);
    }
    ...
    const customerPhone = resolveCustomerNotificationPhone(order.shippingAddress as AddressDto | undefined, user.phone);
  ...
  adminMergeOrders: async (
    ...
    try {
      await session.withTransaction(async () => {
        ...
        if (keepOrder.type === 'delivery' && !areEquivalentAddresses(keepOrder.shippingAddress as AddressDto | undefined, mergeOrder.shippingAddress as AddressDto | undefined)) {
        ...
        const mergedTaxableAmount = Math.max(
          mergedSubtotal + mergedShippingFee - mergedDiscount - mergedLoyaltyDiscount,
          0
        );
        const mergedTaxSnapshot = calculateTaxSnapshot(
          keepOrder.taxRate || mergeOrder.taxRate
            ? {
                enabled: true,
        ...
        const mergedPaymentStatus =
          keepOrder.paymentStatus === 'receipt_uploaded' || mergeOrder.paymentStatus === 'receipt_uploaded' ? 'receipt_uploaded' : 'unpaid';
        ...
        const mergeNote =
          reason?.trim() || `Merged ${mergeOrder.orderNumber} into this order for combined ${keepOrder.type === 'delivery' ? 'shipping' : 'pickup'}`;
    ...
    logger.info(
      `order.admin_merged keep=${mergedOrderToReturn._id.toString()} merged=${mergeOrderId} actor=${actorName} total=${mergedOrderToReturn.total}`
    );
    await invalidateAnalyticsCache();
    return serializeOrder(mergedOrderToReturn.toObject());
  },

  softDeleteOrder: async (orderId: string): Promise<void> => {
  ...
  exportOrdersCsv: async (): Promise<string> => {
    ...
    for (const order of orders) {
      const populatedUser = order.user as unknown as { name?: string; email?: string } | undefined;
      rows.push(
        [
