...
const invalidateCachedUser = async (userId, options) => {
...
const listManualExpensesWithFallback = async (config) => {
    const expenses = await ManualExpense.find().sort({ incurredOn: -1, createdAt: -1 }).lean();
    if (expenses.length > 0) {
        return expenses;
    }
    const legacyExpenses = getLegacyManualExpenses(config);
    if (legacyExpenses.length > 0) {
        await ManualExpense.bulkWrite(legacyExpenses.map((expense) => ({
            updateOne: {
...
const PAID_ORDER_ANALYTICS_DATE_FIELD = 'analyticsPaidAt';
const PAID_ORDER_ANALYTICS_DATE_PATH = `$${PAID_ORDER_ANALYTICS_DATE_FIELD}`;
const buildPaidOrderAnalyticsStages = (extraMatch = {}, range) => {
    const stages = [
        {
            $match: {
                deletedAt: null,
        ...
        {
            $addFields: {
                [PAID_ORDER_ANALYTICS_DATE_FIELD]: {
    ...
    if (range) {
        stages.push({
            $match: {
                [PAID_ORDER_ANALYTICS_DATE_FIELD]: range
            }
        });
    }
    return stages;
...
export const adminService = {
    ...
    getAnalytics: async (query = {}) => {
        ...
        return cacheService.rememberVersioned(cacheNamespaces.analytics, cacheKey, ANALYTICS_CACHE_TTL_SECONDS, async () => {
            ...
            const [currentOrderStats, comparisonOrderStats, revenueByDay, statusCounts, monthlyRevenue, currentTopProducts, comparisonTopProducts, geographicDistribution, cartActivityResult, quotationCountResult, confirmedOrdersResult, userAnalyticsResult, lowStockAlerts] = await Promise.all([
                Order.aggregate([
                    ...buildPaidOrderAnalyticsStages({}, currentRange),
                    { $group: { _id: null, revenue: { $sum: '$total' }, orders: { $sum: 1 } } }
                ]),
                Order.aggregate([
                    ...buildPaidOrderAnalyticsStages({}, comparisonRange),
                    { $group: { _id: null, revenue: { $sum: '$total' }, orders: { $sum: 1 } } }
                ]),
                Order.aggregate([
                    ...buildPaidOrderAnalyticsStages({}, currentRange),
                    {
                        $group: {
                            _id: { $dateToString: { format: '%Y-%m-%d', date: PAID_ORDER_ANALYTICS_DATE_PATH } },
                            revenue: { $sum: '$total' }
                        }
                    },
                    { $sort: { _id: 1 } }
                ...
                Order.aggregate([
                    ...buildPaidOrderAnalyticsStages({}, { $gte: last12Months[0]?.toDate(), $lte: analyticsWindow.currentEnd.toDate() }),
                    {
                        $group: {
                            _id: { $dateToString: { format: '%Y-%m', date: PAID_ORDER_ANALYTICS_DATE_PATH } },
                            revenue: { $sum: '$total' }
                        }
                    },
                    { $sort: { _id: 1 } }
                ...
                Order.aggregate([
                    ...buildPaidOrderAnalyticsStages({ 'shippingAddress.district': { $exists: true, $nin: ['', null] } }, currentRange),
                    {
                        $group: {
    ...
    getSalesAnalysis: async () => cacheService.rememberVersioned(cacheNamespaces.analytics, 'sales-analysis', ANALYTICS_CACHE_TTL_SECONDS, async () => {
        ...
        const [orderAnalyticsResult, customerAnalyticsResult, customerRfmResult, customerRetentionActivityResult, customerMining] = await Promise.all([
            Order.aggregate([
                ...buildPaidOrderAnalyticsStages(),
                {
                    $facet: {
                        daily: [
                            { $match: { [PAID_ORDER_ANALYTICS_DATE_FIELD]: { $gte: last30Start, $lte: todayEnd.toDate() } } },
                            {
                                $group: {
                                    _id: { $dateToString: { format: '%Y-%m-%d', date: PAID_ORDER_ANALYTICS_DATE_PATH } },
                                    revenue: { $sum: '$total' },
                        ...
                        monthly: [
                            { $match: { [PAID_ORDER_ANALYTICS_DATE_FIELD]: { $gte: monthlyWindowStart, $lte: todayEnd.toDate() } } },
                            {
                                $group: {
                                    _id: { $dateToString: { format: '%Y-%m', date: PAID_ORDER_ANALYTICS_DATE_PATH } },
                                    revenue: { $sum: '$total' },
                        ...
                        yearly: [
                            { $match: { [PAID_ORDER_ANALYTICS_DATE_FIELD]: { $gte: yearlyWindowStart, $lte: todayEnd.toDate() } } },
                            {
                                $group: {
                                    _id: { $dateToString: { format: '%Y', date: PAID_ORDER_ANALYTICS_DATE_PATH } },
                                    revenue: { $sum: '$total' },
                                    orderCount: { $sum: 1 }
                                }
                            }
                        ...
                        todayStats: [
                            { $match: { [PAID_ORDER_ANALYTICS_DATE_FIELD]: { $gte: todayStart.toDate(), $lte: todayEnd.toDate() } } },
                            { $group: { _id: null, revenue: { $sum: '$total' }, orderCount: { $sum: 1 } } }
                        ],
                        monthToDateStats: [
                            { $match: { [PAID_ORDER_ANALYTICS_DATE_FIELD]: { $gte: monthStart.toDate(), $lte: todayEnd.toDate() } } },
                            { $group: { _id: null, revenue: { $sum: '$total' }, orderCount: { $sum: 1 } } }
                        ],
                        yearToDateStats: [
                            { $match: { [PAID_ORDER_ANALYTICS_DATE_FIELD]: { $gte: yearStart.toDate(), $lte: todayEnd.toDate() } } },
                            { $group: { _id: null, revenue: { $sum: '$total' }, orderCount: { $sum: 1 } } }
                        ]
                    }
                }
            ...
            Order.aggregate([
                ...buildPaidOrderAnalyticsStages(),
                {
                    $group: {
                        ...
                        orderCount: { $sum: 1 },
                        lastOrderAt: { $max: PAID_ORDER_ANALYTICS_DATE_PATH },
                        firstOrderAt: { $min: PAID_ORDER_ANALYTICS_DATE_PATH }
                    }
                }
            ]),
            Order.aggregate([
                ...buildPaidOrderAnalyticsStages({}, { $gte: cohortWindowStart.toDate(), $lte: todayEnd.toDate() }),
                {
                    $group: {
                        _id: {
                            user: '$user',
                            month: { $dateToString: { format: '%Y-%m', date: PAID_ORDER_ANALYTICS_DATE_PATH } }
                        }
                    }
                },
                { $sort: { '_id.month': 1 } }
            ]),
            customerBehaviorService.getCustomerMining()
        ]);
    ...
    createExternalExpense: async (payload) => {
        await listManualExpensesWithFallback(await siteConfigService.getOrCreateDocument());
    ...
    deleteExternalExpense: async (expenseId) => {
        ...
        const deletedExpense = await ManualExpense.findByIdAndDelete(expenseId);
        if (!deletedExpense) {
            throw new AppError('Expense entry not found', 404);
        }
        if ((await ManualExpense.estimatedDocumentCount()) === 0) {
