...
import { ReviewHelpfulVote, type ReviewHelpfulVoteDocument } from '../../models/ReviewHelpfulVote.js';
...
import {
  ...
  mergeUniqueObjectIds,
  mergeUniqueText,
  normalizeComparableText,
  syncReviewRatingsForProducts,
  SELF_MANAGED_ADMIN_PERMISSIONS
} from './adminShared.js';

const emptyAdminUserOrderStats: AdminUserOrderStatsDto = {
  totalOrders: 0,
  totalSpend: 0
};

const buildAdminUserOrderStatsMap = async (userIds: Types.ObjectId[]): Promise<Map<string, AdminUserOrderStatsDto>> => {
  if (userIds.length === 0) {
    return new Map();
  }
...
const mergeAddresses = (left: Array<any> = [], right: Array<any> = []): any[] => {
  const mergedAddresses: any[] = [];
...
const mergeCartItems = (
  left: Array<{ product: { toString: () => string }; variantIndex?: number | null; quantity: number }> = [],
  right: Array<{ product: { toString: () => string }; variantIndex?: number | null; quantity: number }> = []
): Array<{ product: { toString: () => string }; variantIndex?: number | null; quantity: number }> => {
  const merged = new Map<string, { product: { toString: () => string }; variantIndex?: number | null; quantity: number }>();
...
export const userAdminService = {
  ...
  listUserLoginHistory: async (userId: string, limit = 10): Promise<AdminUserLoginEntryDto[]> => {
    ...
    if (loginEvents.length > 0) {
      return loginEvents.map((entry) => {
        const metadata = (entry.metadata ?? {}) as { rememberMe?: unknown };
  ...
  mergeUsers: async (keepUserId: string, mergeUserId: string, actingUserId: string): Promise<AdminUserMergeResultDto> => {
    ...
    let mergeResult: AdminUserMergeResultDto | null = null;
    const affectedProductIds: Types.ObjectId[] = [];
    ...
    try {
      await session.withTransaction(async () => {
        ...
        const affectedHelpfulReviewIds = new Set<string>();

        const upsertHelpfulVoteReference = async (vote: HydratedDocument<ReviewHelpfulVoteDocument>, reviewId: Types.ObjectId): Promise<void> => {
          if (!vote) {
            return;
          }
        ...
        const transferHelpfulVotes = async (fromReviewId: Types.ObjectId, toReviewId = fromReviewId): Promise<void> => {
        ...
        const retargetHelpfulVotesForMergedUser = async (): Promise<void> => {
          const votes = await ReviewHelpfulVote.find({ user: mergeUser._id }).session(session);
          for (const vote of votes) {
            await upsertHelpfulVoteReference(vote, vote.review as Types.ObjectId);
          }
        };

        const syncHelpfulVoteCounts = async (): Promise<void> => {
        ...
        for (const review of mergeReviews) {
          affectedProductIds.push(review.product as Types.ObjectId);
        ...
        const mergedWishlistItems = mergeUniqueObjectIds(
          mergeUniqueObjectIds(keepUser.wishlist ?? [], keepWishlist?.items ?? []),
          mergeUniqueObjectIds(mergeUser.wishlist ?? [], mergeWishlist?.items ?? [])
        );
        ...
        if (keepCart) {
          keepCart.items = mergedCartItems as any;
  ...
  updateUser: async (
    userId: string,
    payload: Partial<{ role: 'customer' | 'staff' | 'admin'; isActive: boolean; permissions: AdminPermission[] }>,
    actingUserId: string
  ): Promise<UserSummary> => {
  ...
  removeUser: async (userId: string, actingUserId: string): Promise<void> => {
    if (userId === actingUserId) {
      throw new AppError('You cannot deactivate your own account', 400);
    }
  ...
  permanentlyDeleteUser: async (userId: string, actingUserId: string): Promise<void> => {
    if (userId === actingUserId) {
