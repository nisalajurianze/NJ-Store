...
import type { AdminReturnRequestDto, ImageAsset, PaginatedResult, ReturnRequestDto } from '@njstore/types';
...
type AdminReturnRequestSerializerInput = Parameters<typeof serializeAdminReturnRequest>[0];
...
const normalizeVariantIndex = (value: number | null | undefined): number | undefined =>
  typeof value === 'number' && Number.isInteger(value) && value >= 0 ? value : undefined;

const makeRequestedItemKey = (item: Pick<ReturnRequestItemInput, 'sku' | 'variantIndex'>): string =>
  `${item.sku.trim()}::${item.variantIndex ?? 'default'}`;

const orderItemToRequestInput = (item: { sku: string; quantity: number; variantIndex?: number | null }): ReturnRequestItemInput => ({
  sku: item.sku,
...
const resolveRefundTotals = (
  selectedSubtotal: number,
  payload: { refundAmount?: number; refundPercent?: number }
...
const buildReturnSelection = (
  orderItems: OrderReturnItemSource[],
  payload: {
    items?: ReturnRequestItemInput[];
    refundAmount?: number;
    refundPercent?: number;
...
const createReturnNotification = async (
  userId: string,
  payload: {
    type: ReturnRequestDto['status'] | 'created';
    orderId: string;
    orderNumber: string;
    adminNote?: string;
  }
...
export const returnService = {
  createRequest: async (userId: string, orderId: string, payload: CreateReturnRequestInput): Promise<ReturnRequestDto> => {
    ...
    const selection = buildReturnSelection(order.items as unknown as OrderReturnItemSource[], payload);
  ...
  listForOrder: async (userId: string, orderId: string): Promise<ReturnRequestDto[]> => {
  ...
  uploadEvidence: async (
  ...
  ): Promise<ReturnRequestDto | AdminReturnRequestDto> => {
    ...
    const uploadedAssets: ImageAsset[] = await Promise.all(
      files.map((file) =>
        uploadBuffer({
          file,
          folder: 'returns',
    ...
    if (uploadedBy === 'admin') {
      const updatedRequest = await ReturnRequest.findById(returnRequestId)
        ...
        .lean<AdminReturnRequestSerializerInput | null>();
  ...
  listAll: async (filters: {
    ...
    status?: string;
    search?: string;
  }): Promise<PaginatedResult<AdminReturnRequestDto>> => {
    ...
    const [requests, total] = await Promise.all([
      ReturnRequest.find(query)
        ...
        .skip((page - 1) * limit)
        .limit(limit)
        .lean<AdminReturnRequestSerializerInput[]>(),
    ...
    try {
      await session.withTransaction(async () => {
        ...
        if (payload.status !== 'rejected' && (payload.items?.length || payload.refundAmount !== undefined || payload.refundPercent !== undefined)) {
          const selection = buildReturnSelection(order.items as unknown as OrderReturnItemSource[], {
            items:
              payload.items ??
