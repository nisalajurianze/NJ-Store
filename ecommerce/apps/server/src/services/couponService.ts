...
export interface CouponValidationResult extends CouponApplicationDto {
  couponId: string;
}

export interface ResolvedCouponResult {
  couponId?: string;
  code?: string;
  discount: number;
  finalTotal: number;
...
const hydrateCouponLineItems = async (
  items: CouponSelectionInput['items']
...
const buildFallbackCouponLineItems = (subtotal: number): CouponLineItem[] => [
  {
    productId: 'subtotal',
    quantity: 1,
    unitPrice: subtotal,
...
const hasCompletedOrderHistory = async (userId: string): Promise<boolean> =>
  Boolean(
    await Order.exists({
      user: userId,
      isQuotation: false,
...
const getUserCouponUsageCount = async (
  couponId: string,
  userId: string
): Promise<number> => CouponUsage.countDocuments({ coupon: new Types.ObjectId(couponId), user: new Types.ObjectId(userId) });

const filterEligibleLineItems = (
  coupon: {
    appliesToCategories?: Array<{ toString: () => string }>;
    appliesToBrands?: Array<{ toString: () => string }>;
  },
...
const calculateCouponDiscount = (
  ...
  subtotal: number,
  shippingFee: number
): Omit<CouponApplicationDto, 'couponId' | 'code'> => {
  if (!lineItems.length) {
...
const assertCouponEligibility = async (
  coupon: {
    _id: Types.ObjectId;
    code: string;
    expiryDate: Date;
...
const buildResult = (
  ...
  outcome: Omit<CouponApplicationDto, 'couponId' | 'code'>,
  autoApplied = false
...
const findBestAutoCoupon = async (
  payload: Omit<CouponSelectionInput, 'code'>,
  lineItems: CouponLineItem[]
  ...
  let bestCoupon: CouponValidationResult | null = null;
...
export const couponService = {
  validateCoupon: async ({
    ...
    shippingFee = 0,
    items
  }: CouponSelectionInput): Promise<CouponValidationResult> => {
    if (!code?.trim()) {
  ...
  resolveBestDiscount: async ({
    ...
    shippingFee = 0,
    items
  }: CouponSelectionInput): Promise<ResolvedCouponResult> => {
    const hydratedLineItems = await hydrateCouponLineItems(items);
    const lineItems = hydratedLineItems.length ? hydratedLineItems : buildFallbackCouponLineItems(subtotal);
  ...
  consumeCoupon: async (couponId: string, userId: string, session?: mongoose.ClientSession): Promise<void> => {
  ...
  restoreCoupon: async (code: string | undefined, userId: string, session?: mongoose.ClientSession): Promise<void> => {
    if (!code) {
      return;
    }
