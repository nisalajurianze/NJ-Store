...
const getProductStock = (product: ProductLike, variantIndex?: number): number => {
...
export const backInStockService = {
  subscribe: async (payload: {
    ...
    const product = await Product.findById(payload.productId)
      ...
      .lean<(ProductLike & { isActive: boolean }) | null>();
  ...
  notifyOnStockTransition: async (before: ProductLike | null | undefined, after: ProductLike | null | undefined): Promise<void> => {
    if (!before || !after) {
      return;
    }
    ...
    const restockedVariantIndexes =
      after.productType === 'standard'
        ? after.variants
            .map((variant, index) => ({
              index,
    ...
    for (const subscription of subscriptions) {
      const shouldNotify =
        subscription.variantIndex !== undefined
      ...
      const variant =
        subscription.variantIndex !== undefined ? after.variants[subscription.variantIndex] : undefined;

      await emailService.sendBackInStock(
        subscription.name?.trim() || 'there',
        subscription.email,
        after.name,
        productUrl,
