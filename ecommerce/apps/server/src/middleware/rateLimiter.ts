...
type SkipHandler = (req: Request, res: Response) => boolean | Promise<boolean>;

const normalizeIp = (value: string | undefined): string => value?.trim().toLowerCase().replace(/^::ffff:/, '') ?? '';
...
class ResilientRedisStore implements Store {
  ...
  private logRedisFallback(error: unknown): void {
    ...
    logger.warn(
      `rate_limit.redis.unavailable ${error instanceof Error ? error.message : 'Redis command failed'}; falling back to in-memory rate limits`
    );
  }
...
const buildRateLimitStore = (keyPrefix?: string): Store | undefined => {
  if (!env.REDIS_URL) {
...
export const shouldBypassRateLimitInLocalDevelopment = (req: Pick<Request, 'ip' | 'socket'>): boolean => {
...
const buildRateLimiter = (options: {
  ...
  return rateLimit({
    ...
    legacyHeaders: false,
    skip: async (req, res) =>
      shouldBypassRateLimitInLocalDevelopment(req) || Boolean(options.skip && (await options.skip(req, res))),
    keyGenerator: options.keyGenerator ?? ((req: Request) => normalizeIp(req.ip ?? req.socket?.remoteAddress) || 'unknown'),
    handler: (_req, res) => {
      res.status(429).json({
        success: false,
...
export const globalRateLimiter = buildRateLimiter({
  ...
  productionMax: 300,
  developmentMax: 5000,
  skip: (req: Request) => req.originalUrl === '/api/v1/health'
});
...
export const searchRateLimiterWhenQueryPresent: RequestHandler = (req, res, next) => {
