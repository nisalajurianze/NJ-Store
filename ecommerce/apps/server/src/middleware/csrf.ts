...
const signTokenSeed = (seed: string): string =>
  crypto.createHmac('sha256', env.JWT_ACCESS_SECRET).update(seed).digest('base64url');
...
const isValidCsrfToken = (token: string | undefined): boolean => {
  if (!token) {
    return false;
  }
...
export const requireAuthCsrfToken = (req: Request, _res: Response, next: NextFunction): void => {
