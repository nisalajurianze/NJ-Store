  ...
  sendResponse(
    ...
    items.map((item) => {
      ...
      return {
        ...serialized,
        brand:
          (typeof serialized.brand === 'string' ? serialized.brand : serialized.brand?.name)
...
export const bulkAdjustProductPrices = catchAsync(async (req, res) => {
  ...
  sendResponse(
    ...
    200,
    data,
    `Adjusted ${data.updatedCount} products${data.flashDealsDisabledCount > 0 ? ` and disabled ${data.flashDealsDisabledCount} flash deals` : ''}.`
  );
});
