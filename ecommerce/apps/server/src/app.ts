...
const getForwardedProto = (value: string | string[] | undefined): string | undefined => {
...
app.use((req, res, next) => {
  ...
  const isSensitiveRoute =
    sensitiveRoutePrefixes.some((prefix) => req.path.startsWith(prefix)) || req.path === '/api/v1/coupons/apply';
...
app.get(/^\/uploads\/(.+)$/, (req, res, next) => {
  ...
  res.sendFile(resolveLocalAssetPath(`local:${relativePath}`), (error?: (NodeJS.ErrnoException & { statusCode?: number }) | null) => {
    if (!error) {
      return;
    }
