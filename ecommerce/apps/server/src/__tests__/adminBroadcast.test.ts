...
const createCustomer = async (overrides: Partial<{ email: string; isActive: boolean; isEmailVerified: boolean }> = {}) =>
  User.create({
    name: 'Customer User',
...
describe('Admin broadcast emails', () => {
  it(
    'returns verified recipient counts for the broadcast audience summary',
    async () => {
  ...
  it(
    'sends a deduplicated broadcast across verified customers and confirmed newsletter subscribers',
    async () => {
      ...
      expect(sentPayloads.map((payload) => payload.to).sort()).toEqual(
        ['shared@example.com', 'customer-only@example.com', 'unverified-only@example.com', 'newsletter-only@example.com'].sort()
      );
  ...
  it(
    'sends a broadcast only to active unverified customers when that audience is selected',
    async () => {
      ...
      const sentPayloads = vi.mocked(emailService.sendAdminBroadcast).mock.calls.map(([payload]) => payload);
      expect(sentPayloads.map((payload) => payload.to).sort()).toEqual(
        ['unverified-one@example.com', 'unverified-two@example.com'].sort()
      );
  ...
  it(
    'requires setting write access to send a broadcast',
    async () => {
