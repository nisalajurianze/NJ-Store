...
vi.mock('../services/uploadService.js', () => ({
  uploadBuffer: vi.fn(async ({ file, folder }: { file: Express.Multer.File; folder: string }) => ({
    url: `http://mock-url.com/${folder}/${file.originalname}`,
    publicId: `mock:${folder}/${file.originalname}`,
    alt: file.originalname
  })),
  uploadGeneratedBuffer: vi.fn(async ({ fileName, folder }: { fileName: string; folder: string }) => ({
  ...
  resolveLocalAssetPath: vi.fn((publicId: string) => publicId),
  resolveAssetDeliveryUrl: vi.fn((asset: { url: string }) => asset.url)
}));
...
describe('Order Flow', () => {
  ...
  const registerUser = async (
    ...
    return {
      token: registerRes.body.data.tokens.accessToken as string,
      userId: user._id.toString(),
      password
    };
  };
  ...
  const createQuotation = async (
    ...
    return res.body.data as { id: string; quotationToken: string; isQuotation: boolean; total: number };
  };

  const confirmQuotation = async (token: string, quotationToken: string, expectedStatus = 200) =>
    request(app)
  ...
  it(
    'rejects quotations for configurable products when no variant has been selected',
    async () => {
  ...
  it(
    'creates a quotation successfully without requiring email verification',
    async () => {
  ...
  it(
    'sorts customer order listings by latest activity when requested',
    async () => {
      ...
      await Order.collection.updateOne(
        { _id: new mongoose.Types.ObjectId(olderCreatedButRecentlyUpdated.id) },
        {
          $set: {
      ...
      await Order.collection.updateOne(
        { _id: new mongoose.Types.ObjectId(newerCreatedOrder.id) },
        {
          $set: {
      ...
      expect(res.body.data.map((order: { id: string }) => order.id)).toEqual([
  ...
  it(
    'applies configured tax to quotation totals when tax settings are enabled',
    async () => {
  ...
  it(
    'stores the selected cash on delivery payment method on quotation creation',
    async () => {
  ...
  it(
    'creates a quotation before fulfilment is chosen and lets the customer confirm it later with delivery details',
    async () => {
      ...
      const quotationToken = quotationRes.body.data.quotationToken as string;
  ...
  it(
    'requires verified email before confirming a quotation into an order',
    async () => {
  ...
  it(
    'uploads multiple receipts and removes a single receipt while unpaid',
    async () => {
      ...
      const orderId = confirmed.body.data.id as string;
      ...
      const firstReceiptId = uploadRes.body.data.receipts[0].id as string;
  ...
  it(
    'rejects receipt changes after admin marks the order paid',
    async () => {
      ...
      const receiptId = paidRes.body.data.receipts[0].id as string;
  ...
  it(
    'rejects receipt changes after the order is cancelled',
    async () => {
      ...
      const receiptId = uploadRes.body.data.receipts[0].id as string;
  ...
  it(
    'allows admin to mark an order as paid when no receipt exists',
    async () => {
  ...
  it(
    'only exposes the invoice after admin marks the order as paid',
    async () => {
      const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
        new Response(Buffer.from('mock proxied invoice pdf'), {
          status: 200,
          headers: {
  ...
  it(
    'allows an email-restricted coupon for the matching account',
    async () => {
      await Coupon.create({
        code: 'VIPONLY',
        type: 'fixed',
  ...
  it(
    'rejects an email-restricted coupon for a different account',
    async () => {
      await Coupon.create({
        code: 'VIPONLY',
        type: 'fixed',
  ...
  it(
    'restores inventory and coupon usage when a confirmed order is cancelled by the customer',
    async () => {
      await Coupon.create({
        code: 'SAVE10',
        type: 'percentage',
      ...
      const quotation = quotationRes.body.data as { id: string; quotationToken: string };
      const confirmed = await confirmQuotation(customer.token, quotation.quotationToken);
      const orderId = confirmed.body.data.id as string;
  ...
  it(
    'redeems loyalty points when a quotation is confirmed and returns them when the order is cancelled',
    async () => {
  ...
  it(
    'refreshes cached product stock and low-stock analytics after confirmation and cancellation',
    async () => {
      ...
      expect(analyticsAfterConfirm.body.data.lowStockAlerts).toEqual(
  ...
  it(
    'refreshes cached product stock and low-stock analytics after admin cancellation',
    async () => {
      ...
      expect(warmedAnalytics.body.data.lowStockAlerts).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            productId,
            variantSku: 'TEST-1',
  ...
  it(
    'keeps quotations out of admin order management',
    async () => {
      ...
      expect(listRes.body.data.some((entry: { id: string }) => entry.id === quotation.id)).toBe(false);
  ...
  it(
    'includes customer details in the admin order list response',
    async () => {
      ...
      const order = listRes.body.data.find((entry: { orderNumber: string }) => entry.orderNumber.startsWith('ORD-'));
  ...
  it(
    'lets admins create a manual delivery order with customer and fulfilment details',
    async () => {
      ...
      const listedOrder = listRes.body.data.find((entry: { id: string }) => entry.id === manualOrder.id);
  ...
  it(
    'lets admins assign an order to active staff and exposes the assignee in the admin order list response',
    async () => {
      ...
      const order = listRes.body.data.find((entry: { id: string }) => entry.id === orderId);
  ...
  it(
    'sends a shipping notification email once an order has been shipped',
    async () => {
      ...
      const order = updatedOrder.body.data.find((entry: { id: string }) => entry.id === orderId);
      ...
      expect(order.timeline.some((entry: { note: string }) => /Shipping notification sent to customer/i.test(entry.note))).toBe(true);
    },
    TEST_TIMEOUT
  );

  it(
    'merges two compatible orders from the same customer into the kept order',
    async () => {
      ...
      const keepOrderId = firstOrder.body.data.id as string;
      const mergeOrderId = secondOrder.body.data.id as string;
      ...
      const keptOrder = listRes.body.data.find((entry: { id: string }) => entry.id === keepOrderId);
      const removedOrder = listRes.body.data.find((entry: { id: string }) => entry.id === mergeOrderId);
  ...
  it(
    'grants loyalty points and records the loyalty transaction when an order is delivered',
    async () => {
