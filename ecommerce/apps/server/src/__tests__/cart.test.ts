...
describe('Cart Flow', () => {
  it(
    'rejects adding configurable products without a selected variant',
    async () => {
  ...
  it(
    'normalizes legacy single-variant guest cart items when the cart is read',
    async () => {
      const product = await Product.create({
  ...
  it(
    'drops ambiguous legacy multi-variant guest cart items when the cart is read',
    async () => {
      const product = await Product.create({
