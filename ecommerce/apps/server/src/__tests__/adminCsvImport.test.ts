...
describe('Admin CSV product import', () => {
  it(
    'imports export-style CSV rows into valid products',
    async () => {
      ...
      expect((product?.brand as { name?: string } | null)?.name).toBe('Dell');
      ...
      expect((product?.category as { name?: string } | null)?.name).toBe('Laptops');
    },
    TEST_TIMEOUT
  );

  it(
    'updates an existing single-variant product when the CSV row reuses its SKU',
    async () => {
      ...
      expect((product?.brand as { name?: string } | null)?.name).toBe('Logitech');
  ...
  it(
    'creates missing brand records from imported CSV rows',
    async () => {
      await Category.create({
        name: 'Audio',
        slug: 'audio',
