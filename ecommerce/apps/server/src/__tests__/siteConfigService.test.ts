...
describe('siteConfigService.updateConfig', () => {
  it(
    'persists storefront maintenance, tax settings, currencies, notification settings, social links, and email templates',
    async () => {
  ...
  it(
    'backfills legacy email templates into the dedicated collection',
    async () => {
  ...
  it(
    'rejects stale admin settings revisions',
    async () => {
  ...
  it(
    'rejects unsafe map embed URLs and sanitizes legacy stored values',
    async () => {
      await expect(
        StoreSetting.create({
          footer: {
      ...
      await StoreSetting.collection.updateOne(
