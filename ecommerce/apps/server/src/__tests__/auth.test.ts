...
describe('Authentication Flow', () => {
  ...
  const getCookies = (value: string[] | string | undefined): string[] => {
  ...
  it(
    'should login with Google credentials and provision a verified customer session',
    async () => {
      const res = await request(app)
  ...
  it(
    'should get profile with valid access token',
    async () => {
  ...
  it(
    'should persist saved shop filters on the user profile',
    async () => {
  ...
  it(
    'should reject an access token after logout revokes its session',
    async () => {
      const registerRes = await request(app)
        .post('/api/v1/auth/register')
        .send(testUser)
  ...
  it(
    'should reject an old access token after the password is changed',
    async () => {
      const registerRes = await request(app)
        .post('/api/v1/auth/register')
        .send(testUser)
