...
describe('Product listing', () => {
  it(
    'caches the public home feed until catalog cache namespaces change',
    async () => {
  ...
  it(
    'returns public products when query validation is applied',
    async () => {
      const category = await Category.create({
        name: 'Smartphones',
        slug: 'smartphones',
  ...
  it(
    'sanitizes unsafe query keys without failing under Express 5',
    async () => {
      const category = await Category.create({
        name: 'Printers',
        slug: 'printers',
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.data.some((product: { name: string }) => product.name === 'LaserJet Pro')).toBe(true);
    },
    TEST_TIMEOUT
  );

  it(
    'filters flash deals to active countdown-capable or evergreen entries',
    async () => {
      const category = await Category.create({
        name: 'Accessories',
        slug: 'accessories',
  ...
  it(
    'returns a compact public price range without fetching product cards',
    async () => {
      const category = await Category.create({
        name: 'Laptops',
        slug: 'laptops',
  ...
  it(
    'excludes requested products from public related-product lists',
    async () => {
      const category = await Category.create({
        name: 'Monitors',
        slug: 'monitors',
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.data.map((product: { id: string }) => product.id)).toEqual([relatedProduct._id.toString()]);
    },
    TEST_TIMEOUT
  );

  it(
    'returns curated upsells in one request and excludes cart items',
    async () => {
      const category = await Category.create({
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.data.map((product: { id: string }) => product.id)).toEqual([accessory._id.toString()]);
    },
    TEST_TIMEOUT
  );

  it(
    'prioritizes accessory add-ons over similar main products for cart upsells',
    async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.data.map((product: { id: string }) => product.id)).toEqual([accessory._id.toString()]);
      expect(response.body.data.map((product: { id: string }) => product.id)).not.toContain(similarPhone._id.toString());
    },
    TEST_TIMEOUT
  );
});
