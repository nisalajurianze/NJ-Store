...
describe('Admin product routes', () => {
  it(
    'accepts relative canonical URLs and clears optional fields when updating through the admin API',
    async () => {
  ...
  it(
    'bulk-adjusts product prices and restores an older version through the admin API',
    async () => {
      ...
      const initialVersionId = versionsAfterAdjustment.body.data.find((version: { version: number }) => version.version === 1)?.id;
