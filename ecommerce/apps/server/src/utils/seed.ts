...
const image = (folder: string, slug: string, alt: string) => ({
...
const buildVariants = (sku: string, basePrice: number) => [
...
const buildSpecifications = (name: string) => [
...
const buildAddress = (name: string, phone: string, city: string) => ({
  label: 'Home',
  fullName: name,
  phone,
...
const createSeedPassword = (): string => `Nj-${randomBytes(9).toString('base64url')}!A1`;

const resolveSeedPassword = (
  envKey: typeof SEED_ADMIN_PASSWORD_ENV_KEY | typeof SEED_CUSTOMER_PASSWORD_ENV_KEY
): { value: string; source: 'env' | 'generated' } => {
...
const createOrders = async (users: SeedUser[], products: SeedProduct[]) => {
...
export const seedDatabase = async (): Promise<void> => {
  ...
  if (hasExistingData && !shouldReset) {
    console.warn(
      `[seed] Existing data detected (users=${existingCounts.users}, categories=${existingCounts.categories}, products=${existingCounts.products}, orders=${existingCounts.orders}, banners=${existingCounts.banners}, brands=${existingCounts.brands}).`
    );
  ...
  await Banner.create({
    ...
    subtitle:
      'Live hero banners can now be managed from the admin dashboard, while shoppers still get the same quotation-first experience.',
  ...
  if (adminPassword.source === 'generated' || customerPassword.source === 'generated') {
    console.log(
      `[seed] Set ${SEED_ADMIN_PASSWORD_ENV_KEY} and ${SEED_CUSTOMER_PASSWORD_ENV_KEY} in your local server .env if you want stable demo credentials.`
    );
  }
  await mongoose.disconnect();
};
