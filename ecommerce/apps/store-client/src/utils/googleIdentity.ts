...
const markScriptLoaded = (script: HTMLScriptElement): void => {
  ...
  if (!googleIdentityScriptPromise) {
    googleIdentityScriptPromise = new Promise((resolve, reject) => {
      ...
      const resolveWhenReady = (): void => {
        ...
        window.clearTimeout(timeoutId);
        googleIdentityScriptPromise = null;
        reject(new Error('Google Identity Services loaded without initializing the client API.'));
      };

      const handleLoad = (event: Event): void => {
        markScriptLoaded(event.currentTarget as HTMLScriptElement);
        resolveWhenReady();
      };
      ...
      if (existingScript) {
        const isEnglishScript = existingScript.src.includes('hl=en');

        if (!isEnglishScript) {
          existingScript.remove();
        } else if (existingScript.dataset.googleIdentityLoaded === 'true') {
          resolveWhenReady();
          return;
        } else {
